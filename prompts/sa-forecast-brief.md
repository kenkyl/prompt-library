...
# No `scheduled: true`. ~/Claude/Scheduled/<name>/SKILL.md is GENERATED by the
# desktop app from each scheduled task, not read by it -- installing there
# creates a directory the app does not recognise as a task. A scheduled task
# carries its prompt inline in its Instructions field, so the delivery route
# for that surface is the `instructions` build output, pasted in.
